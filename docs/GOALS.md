...
**卡在"需你拍板的设计/数据",Claude 不盲写:**
- P1 侧边虚线:画在哪个位置、什么形状?(颜仪晖说"直接复制画虚线过来")
- P4 单/双击纹样:双击用**哪张**纹样贴图?**哪些音符**算双击?(现有:单击=hit_block、滑动=frog_swipe蛙纹、长按=snake_done蛇纹,无 Double 种类)
- P6 双/长按区分:需新增 Double 音符种类 + 双击输入判定 + 上面 P4 的贴图映射
- 颜仪晖倾向:临近 8/20,纹样玩法**可能暂不大改**,先对齐形式+补背景,进决赛再议
